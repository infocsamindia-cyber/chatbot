...
import { postToAllPlatforms } from '../services/postingService';
...
export default function AutoMode() {
  ...
  // Use a ref to always have access to the latest posts value inside callbacks
  ...
  // handlePostNow is defined at component level, NOT inside runDailyAuto
  const handlePostNow = useCallback(async () => {
    const currentPosts = postsRef.current;
    if (Object.keys(currentPosts).length === 0) {
      addLog('⚠️ Pehle posts generate karo!', 'warn');
      return;
    }
    setIsPosting(true);
    addLog('📤 Sare platforms pe post ho raha hai...', 'info');

    const results = await postToAllPlatforms(currentPosts);
    setPostingResults(results);

    Object.entries(results).forEach(([platform, result]) => {
      if (result.success) {
        addLog(`✅ ${platform} — post ho gaya!`, 'success');
      } else if (result.needsAuth) {
        addLog(`🔐 ${platform} — login required!`, 'warn');
      } else {
        addLog(`❌ ${platform} — ${result.error}`, 'error');
      }
    });
  ...
  const runDailyAuto = useCallback(async () => {
    if (isRunning) return;
    setIsRunning(true);
...
IMPORTANT — Always mention at least one Ayanix Tech service: Website Development, Mobile App, Custom Software, UI/UX Design, Meta Ads, SEO, E-Commerce
PLATFORM RULES:
- instagram: max 2200 chars + 15 hashtags
- linkedin: professional max 1500 chars, 3-5 hashtags
- twitter: max 280 chars, 2-3 hashtags only
- facebook: friendly max 500 chars
- youtube: video description max 500 chars
- threads: casual max 500 chars
Write ONLY the post. No prefix.`;
    ...
    localStorage.setItem('todaysPosts', JSON.stringify({ date: todayKey, topic, tone, posts: results }));
  ...
  }, [autoEnabled, addLog, runDailyAuto]);
    ...
    <div className="min-h-screen bg-gray-950 text-white overflow-x-hidden">
        ...
        <div className="flex items-center justify-between max-w-5xl">
          <div className="flex-1">
            <div className="flex items-center gap-3 mb-3 flex-wrap">
              ...
              <span className={`text-xs px-3 py-1 rounded-full font-bold border ${autoEnabled ? 'bg-green-500/20 text-green-400 border-green-500/30' : 'bg-gray-700 text-gray-400 border-gray-600'}`}>
            ...
            <div className="flex items-center gap-4 flex-wrap">
              <button onClick={toggleAuto}
                className={`flex items-center gap-3 px-6 py-3 rounded-xl font-bold transition-all ${autoEnabled ? 'bg-green-500 hover:bg-green-600 text-white shadow-lg shadow-green-500/30' : 'bg-gray-700 hover:bg-gray-600 text-white'}`}>
              ...
              <button onClick={runDailyAuto} disabled={isRunning}
                ...
                {isRunning ? <><span className="animate-spin">⚙️</span> {currentStep || 'Running...'}</> : <><span>▶️</span> Run Karo Abhi</>}
              </button>
              {Object.keys(posts).length > 0 && (
                <button
                  onClick={handlePostNow}
                  disabled={isPosting}
                  className="flex items-center gap-2 px-6 py-3 bg-green-600 hover:bg-green-700 disabled:bg-gray-700 disabled:cursor-not-allowed rounded-xl font-bold transition shadow-lg shadow-green-600/30">
                  {isPosting
                    ? <><span className="animate-spin">📤</span> Posting...</>
                    : <><span>📤</span> Post Karo Abhi</>}
                </button>
              )}
            </div>
          </div>
          <div className="hidden lg:block w-48 h-48 flex-shrink-0"><RobotSVG /></div>
        </div>
      </div>
      ...
      <div className="px-8 py-8 max-w-5xl mx-auto">
        ...
        <div className="mb-8 bg-gray-900 border border-gray-800 rounded-2xl p-6">
          <h2 className="text-xl font-bold mb-5">⚡ Kaise Kaam Karta Hai</h2>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {[
              ...
              <div key={s.step} className="relative bg-gray-800 rounded-xl p-4 border border-gray-700">
                <span className={`absolute top-3 right-3 text-xs font-black text-white ${s.color} w-7 h-7 rounded-full flex items-center justify-center`}>{s.step}</span>
        ...
        {autoLog.length > 0 && (
          <div className="mb-8">
            <h2 className="text-xl font-bold mb-3 flex items-center gap-2">
              🖥️ Live Activity Log
              {isRunning && <span className="text-sm bg-green-500/20 text-green-400 px-2 py-0.5 rounded-full animate-pulse border border-green-500/30">● Running</span>}
              {savedToFirebase && <span className="text-sm bg-blue-500/20 text-blue-400 px-2 py-0.5 rounded-full border border-blue-500/30">☁️ Saved</span>}
            </h2>
            <div ref={logRef} className="bg-black border border-gray-800 rounded-2xl p-5 h-56 overflow-y-auto font-mono text-sm">
              {autoLog.map((log, i) => (
                <div key={i} className={`mb-1.5 flex items-start gap-2 ${log.type === 'success' ? 'text-green-400' : log.type === 'error' ? 'text-red-400' : log.type === 'warn' ? 'text-yellow-400' : 'text-gray-400'}`}>
                  <span className="text-gray-600 flex-shrink-0">[{log.time}]</span>
                  <span>{log.msg}</span>
                </div>
              ))}
              {isRunning && <div className="text-purple-400 animate-pulse mt-1">▌ AI working...</div>}
            </div>
          </div>
        )}
          ...
          <div className="mb-8">
            ...
            <div className="grid grid-cols-1 gap-5">
              {PLATFORMS.map((platform, idx) => {
                ...
                return (
                  <div key={platform.id} className={`bg-gradient-to-br ${POST_COLORS[idx]} border border-gray-700/50 rounded-2xl overflow-hidden hover:border-purple-500/40 transition`}>
                    <div className={`bg-gradient-to-r ${platform.color} px-5 py-3 flex items-center justify-between`}>
                      ...
                      <div className="flex gap-2">
                        <button onClick={() => navigator.clipboard.writeText(content)}
                          className="bg-white/20 hover:bg-white/30 px-3 py-1.5 rounded-lg text-sm text-white font-medium transition">📋 Copy</button>
                    ...
                    <div className="p-5 flex gap-5">
                      <div className="flex-1">
                        <p className={`text-gray-200 text-sm leading-relaxed whitespace-pre-wrap ${!isExpanded ? 'line-clamp-4' : ''}`}>{content}</p>
                        {!isExpanded && content.length > 200 && (
                          <button onClick={() => setExpandedPost(platform.id)} className="text-purple-400 text-xs mt-2 hover:text-purple-300">
                        ...
                        {poster ? (
                          <>
                            <div className="relative group">
                              <img src={poster} alt="AI Poster" className="w-32 h-32 rounded-xl object-cover border-2 border-white/20 shadow-xl" />
                            ...
                            <a href={poster} download={`${platform.id}-${Date.now()}.png`}
                              className="text-xs bg-white/10 hover:bg-white/20 text-white px-3 py-1 rounded-lg transition border border-white/10">⬇️ Download</a>
                            <button onClick={() => {
                            ...
                            }} className="text-xs bg-purple-700/40 hover:bg-purple-700/60 text-white px-3 py-1 rounded-lg transition">🎨 New Design</button>
                          </>
                        ) : (
                          <div className="w-32 h-32 rounded-xl border border-gray-700 flex items-center justify-center text-4xl">🖼️</div>
                        )}
                      </div>
                    </div>
                  </div>
        ...
        <div className="bg-gradient-to-br from-blue-900/30 to-purple-900/30 border border-blue-700/40 rounded-2xl p-6">
          <div className="flex items-start gap-4 mb-5">
            ...
            <div>
              ...
              <p className="text-gray-400 text-sm mt-1">Posts generate + Firebase save ho rahi hain. Direct posting ke liye yeh APIs chahiye:</p>
            </div>
          </div>
          <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
            {[
              ...
              <div key={p.name} className="bg-gray-800/60 border border-gray-700 rounded-xl p-4">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className={`ml-auto text-xs px-2 py-0.5 rounded-full ${p.diff === 'easy' ? 'bg-green-500/20 text-green-400' : 'bg-yellow-500/20 text-yellow-400'}`}>
